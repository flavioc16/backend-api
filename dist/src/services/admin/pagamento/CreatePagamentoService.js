...
exports.CreatePagamentoService = void 0;
const prisma_1 = __importDefault(require("../../../prisma"));
class CreatePagamentoService {
    async execute({ valorPagamento, clienteId, userId }) {
        // Verificar se cliente existe
        const clienteExiste = await prisma_1.default.cliente.findUnique({
            where: { id: clienteId },
        });
        if (!clienteExiste)
            throw new Error("Cliente não encontrado");
        // Verificar se usuário existe
        const userExiste = await prisma_1.default.user.findUnique({
            where: { id: userId },
        });
        if (!userExiste)
        ...
        for (const compra of compras) {
            ...
            if (valorRestante >= compra.totalCompra) {
                ...
                await prisma_1.default.compra.update({
                    where: { id: compra.id },
                    data: {
            ...
            await prisma_1.default.pagamento.create({
                data: {
...
exports.CreatePagamentoService = CreatePagamentoService;
